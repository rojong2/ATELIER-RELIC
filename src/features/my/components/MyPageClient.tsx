...
type Props = {
  ...
  initialAddresses: MyAddress[];
  initialOrders: MyOrder[];
  initialWishlist: MyWishlistItem[];
};
...
export default function MyPageClient({
  ...
  }, [userId, initialWishlist, setUserId, setWishlistItems]);

  const [profile, setProfile] = useState<MyProfile>(initialProfile);
  const [showProfileForm, setShowProfileForm] = useState(false);
  const [profileForm, setProfileForm] = useState<MyProfile>(initialProfile);

  const [addresses, setAddresses] = useState<MyAddress[]>(initialAddresses);
  ...
  const [orders] = useState<MyOrder[]>(initialOrders);
  ...
  const handleEdit = (addr: MyAddress) => {
  ...
  const displayWishlist =
    wishlistItems.length > 0 ? wishlistItems : initialWishlist;
